...
export class ProductoRepository {
  ...
  async actualizar(id: string, datos: Partial<{ nombre: string; descripcion: string; precio: number; stock: number; imagenUrl: string }>) {
    return await prisma.producto.update({
      where: { id },
      data: {
        ...datos,
        ...(datos.precio !== undefined && { precio: Number(datos.precio) }),
        ...(datos.stock !== undefined && { stock: Number(datos.stock) })
      }
    });
  }
